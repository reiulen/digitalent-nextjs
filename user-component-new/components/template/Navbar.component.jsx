...
import { Navbar, Nav, NavDropdown, Form, FormControl, Dropdown, Col, Row, Container, Button } from "react-bootstrap";
...
const Navigationbar = ({ session }) => {
  ...
  const { error: errorDataPribadi, dataPribadi } = useSelector((state) => state.getDataPribadi);

  useEffect(() => {
    if (session) {
      if (dataPribadi && Object.keys(dataPribadi).length !== 0 && !dataPribadi.status) {
  ...
  return (
    <>
      <Navbar bg="white" expand="lg" className="shadow header-dashboard d-flex pr-0 px-md-25 px-10" sticky="top">
        <Col sm={12} lg={1} className=" d-flex justify-content-between justify-content-lg-center align-items-center">
          <Navbar.Brand href="/">
            <Image src={`/assets/icon/mainlogo.svg`} width={50} height={50} alt="brand-navbar" />
          </Navbar.Brand>
          <div className="d-flex d-lg-none justify-content-end align-items-center">
            {!isNavOpen && session && session.roles[0] === "user" && (
              <div className="row m-3">
                <a className="col-3 col-xl-4 text-center" onClick={() => setShowSearch(!showSearch)}>
            ...
            <Navbar.Toggle
              ...
              className="p-3"
            >
              {isNavOpen ? <i className="ri-close-line"></i> : <i className="ri-menu-line"></i>}
            </Navbar.Toggle>
          </div>
        </Col>
        {showSearch && (
          <Form className="w-100 my-2 mx-1 row ">
            <div className="position-relative w-100">
              ...
              />
              <IconSearch className="left-center-absolute" style={{ left: "10px" }} />
            </div>
          </Form>
        )}
        <Navbar.Collapse id="basic-navbar-nav" className={`${style.collapsed}`}>
          <Nav className="me-auto dropdown-explore btn-masuk-peserta mr-1 d-lg-block d-none">
            ...
            <div className="row mx-1 my-0">
              <NavDropdown title="Menu" id="basic-nav-dropdown" className="navdropdown-child position-relative w-100 text-menu">
                <Link href="/" passHref>
                  <NavDropdown.Item className="navdropdown-child">Beranda</NavDropdown.Item>
                </Link>
                    ...
                    {akademi.map((item, i) => {
                      return (
                        <Link key={item.id} href={`/detail/akademi/${item.id}`}>
                          <a className="dropdown-item navdropdown-child">{item.slug}</a>
                        </Link>
                      );
                    })}
                  </div>
                ...
                <div className="btn-group dropright w-100">
                  ...
                  <div className="dropdown-menu ml-3">
                    <Link href="/pusat-informasi" passHref>
                      <a className="dropdown-item navdropdown-child">Panduan Test Substansi</a>
                    </Link>
                    <Link href="/pusat-informasi" passHref>
                      <a className="dropdown-item navdropdown-child">Hak dan Kewajiban</a>
                    </Link>
                  </div>
                </div>
                <Link href="/tentang-kami" passHref>
                  <NavDropdown.Item className="navdropdown-child">Tentang Kami</NavDropdown.Item>
                </Link>
                <Link href="/penyelenggara" passHref>
                  <NavDropdown.Item className="navdropdown-child">Penyelenggara</NavDropdown.Item>
                </Link>
                <div className="btn-group dropright w-100">
                  <a
                    type="button"
                ...
                <Link href="/faq" passHref>
                  <NavDropdown.Item className="navdropdown-child">FAQ</NavDropdown.Item>
                </Link>
                <Link href="/kontak" passHref>
                  <NavDropdown.Item className="navdropdown-child">Kontak</NavDropdown.Item>
                </Link>
              </NavDropdown>
            </div>
            {/* END MENU */}
            ...
            {session && session.roles[0] === "user" ? (
              <div>
                <div className="d-lg-none d-block">
                  <div className={`wrap-accouts ${style.wrapAccounts}`} style={{ borderRadius: "20px" }} onClick={() => router.push("/peserta/profile")}>
                    {/* <div className="dot-accouts"></div> */}
                    <Image
                      className="rounded-circle"
                      src={`${dataPribadi && dataPribadi.foto ? dataPribadi.file_path + dataPribadi.foto : "/assets/media/logos/default.png"}`}
                      width={32}
                      height={32}
                      alt="brand-navbar"
                    />
                    <span className="titles-accounts">{dataPribadi?.name || "-"}</span>
                    <IconArrow fill="#6c6c6c" width="14" height="11" className="d-lg-block d-none" style={{ transform: "rotate(90deg)" }} />
                  </div>
                </div>
                <div className="position-relative d-none d-lg-block">
                  <div className={`wrap-accouts ${style.wrapAccounts}`} style={!isShowDropdown ? { borderRadius: "20px" } : {}} onClick={() => setIsShowDropdown(isShowDropdown ? false : true)}>
                    {/* <div className="dot-accouts"></div> */}
                    <Image
                      className="rounded-circle"
                      src={`${dataPribadi && dataPribadi.foto ? dataPribadi.file_path + dataPribadi.foto : "/assets/media/logos/default.png"}`}
                  ...
                  {isShowDropdown && (
                    <ul className="list-wrap-accounts p-0 d-none d-lg-block py-3">
                      ...
                      <li className={`items-lists p-0 p-3`}>
                        <button className={`btn rounded-full ${style.navbar_btnPrimary} d-flex align-items-center justify-content-center`} onClick={() => handlerLogout()}>
          ...
          <Nav style={{ fontSize: "14px", color: "#6C6C6C" }}>
            <Row className="d-lg-none">
                ...
                <Dropdown color="white">
                  ...
                  <Dropdown.Menu>
                    ...
                    {akademi &&
                      akademi.map((item, i) => {
                        return (
                          <Dropdown.Item href={`/detail/akademi/${item.id}`} key={item.id}>
                            {item.slug}
                          </Dropdown.Item>
                        );
                      })}
              ...
              <Col sm={12}>
                <Dropdown color="white">
                  ...
                  <Dropdown.Menu>
                    <Dropdown.Item href="#/action-1">Panduan Test Substansi</Dropdown.Item>
                    <Dropdown.Item href="#/action-2">Hak dan Kewajiban</Dropdown.Item>
                  </Dropdown.Menu>
                </Dropdown>
              </Col>
            ...
            <hr />
            {/* Start side bar */}
            {session && !router.pathname.includes(routerPath) && <Sidebar screenClass={"d-block d-lg-none"} accountFalse={`d-none d-lg-block`} titleAkun={"Lainnya"} />}
            {/* End side bar */}
          </Nav>
        </Navbar.Collapse>
      </Navbar>
