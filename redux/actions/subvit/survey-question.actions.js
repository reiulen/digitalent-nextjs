...
// get all data
export const getAllSurveyQuestionBanks = (page = 1, keyword = '', limit = 5) => async (dispatch) => {
    try {

        dispatch({ type: SURVEY_QUESTION_BANKS_REQUEST })

        let link = process.env.END_POINT_API_SUBVIT + `api/survey-question-banks?page=${page}`
...
export const newSurveyQuestionBanks = (subtanceData) => async (dispatch) => {
    try {
        ...
        const { data } = await axios.post(process.env.END_POINT_API_SUBVIT + 'api/survey-question-banks', subtanceData)
...
export const getDetailSurveyQuestionBanks = (id) => async (dispatch) => {
    try {
        ...
        let link = process.env.END_POINT_API_SUBVIT + `api/survey-question-banks/detail/${id}`
...
export const updateSurveyQuestionBanks = (id, substanceQuestionData) => async (dispatch) => {
    try {
        ...
        const { data } = await axios.post(process.env.END_POINT_API_SUBVIT + `api/survey-question-banks/${id}`, substanceQuestionData)
...
export const updateSurveyQuestionBanksPublish = (subtanceData, id) => async (dispatch) => {
    try {
        ...
        const { data } = await axios.post(process.env.END_POINT_API_SUBVIT + `api/survey-question-banks/publish/${id}`, subtanceData)
