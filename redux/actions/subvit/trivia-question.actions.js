...
// get all data
export const getAllTriviaQuestionBanks = (page = 1, keyword = '', limit = 5) => async (dispatch) => {
    try {

        dispatch({ type: TRIVIA_QUESTION_BANKS_REQUEST })

        let link = process.env.END_POINT_API_SUBVIT + `api/trivia-question-banks?page=${page}`
...
export const newTriviaQuestionBanks = (triviaData) => async (dispatch) => {
    try {
        ...
        const { data } = await axios.post(process.env.END_POINT_API_SUBVIT + 'api/trivia-question-banks', triviaData)
...
export const getDetailTriviaQuestionBanks = (id) => async (dispatch) => {
    try {
        ...
        let link = process.env.END_POINT_API_SUBVIT + `api/trivia-question-banks/detail/${id}`
...
export const updatewTriviaQuestionBanks = (id, triviaData) => async (dispatch) => {
    try {
        ...
        const { data } = await axios.post(process.env.END_POINT_API_SUBVIT + `api/trivia-question-banks/${id}`, triviaData)
...
export const updateTriviaQuestionBanksPublish = (subtanceData, id) => async (dispatch) => {
    try {
        ...
        const { data } = await axios.post(process.env.END_POINT_API_SUBVIT + `api/trivia-question-banks/publish/${id}`, subtanceData)
...
export const allReportTriviaQuestionBanks = (id, page = 1, keyword = '', limit = null) => async (dispatch) => {
    try {

        dispatch({ type: REPORT_TRIVIA_QUESTION_BANKS_REQUEST })

        let link = process.env.END_POINT_API_SUBVIT + `api/trivia-question-banks/report/${id}?page=${page}`
        if (keyword) link = link.concat(`&keyword=${keyword}`)
        if (limit) link = link.concat(`&limit=${limit}`)
