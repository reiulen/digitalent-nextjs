...
import { getEditTrainingStep1, getEditTrainingStep2, getEditTrainingStep3 } from "../../../../redux/actions/pelatihan/training.actions";

const IndexEdit = dynamic(
  () =>
    import(
      "../../../../components/content/pelatihan/training/edit/index-edit"
    ),
  {
...
export const getServerSideProps = wrapper.getServerSideProps(
  (store) =>
    async ({ query, req, params }) => {
      ...
      await store.dispatch(getEditTrainingStep1(query.id, session.user.user.data.token))
      await store.dispatch(getEditTrainingStep2(query.id, session.user.user.data.token))
      await store.dispatch(getEditTrainingStep3(query.id, session.user.user.data.token))
      await store.dispatch(dropdownLevelPelatihan(session.user.user.data.token));
