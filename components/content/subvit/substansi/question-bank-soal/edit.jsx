...
import { clearErrors, updateSubtanceQuestionDetail } from '../../../../../redux/actions/subvit/subtance-question-detail.action'
...
const EditSoalSubstansi = () => {
    ...
    const { loading: detailLoading, error: detailError, subtance_question_detail } = useSelector(state => state.detailSubtanceQuestionDetail)
    const { loading: allLoading, error: allError, subtance_question_type } = useSelector((state) => state.allSubtanceQuestionType)
    const { loading, error, success } = useSelector((state) => state.updateSubtanceQuestionDetail)
    let { id } = router.query

    const [question, setQuestion] = useState(subtance_question_detail.question)
    const [question_image, setQuestionImage] = useState(subtance_question_detail.question_image)

    const [answer, setAnswer] = useState(JSON.parse(subtance_question_detail.answer))

    const [answer_key, setAnswerKey] = useState(subtance_question_detail.answer_key)
    const [question_type, setQuestionType] = useState(subtance_question_detail.type.id)
